...
export const getBikeSalesColumnDefs = (): ColDef<IRow>[] => [
    ...
    },
    {
        headerName: "Customer",
        valueGetter: (params) =>
            params.data?.Customer.first_name !== '' ? `${params.data?.Customer.first_name} ${params.data?.Customer.last_name}` : 'No customer assigned',
        filter: true,
        width: 150
    },
    {
        ...
        cellRenderer: (params: ICellRendererParams) => {
            ...
            return (
                <Stack spacing={0.5} sx={{ py: 1 }} direction={"row"} alignItems="center">
                    ...
                    {bike && <Stack direction="row" spacing={1} alignItems="center">
                        {bike.bike_type && (
                        ...
                        {bike.condition && (
                            <Chip
                                ...
                                color={
                                    bike.condition === 'New' ? 'success' :
                                        bike.condition === 'Refurbished' ? 'warning' : 'default'
                                }
                            />
                        )}
                    </Stack>}
    ...
    {
        headerName: "Price",
        valueGetter: (params) => params.data?.Bike?.price || 0,
        cellRenderer: (params: ICellRendererParams) => {
            const price = params.data?.Bike?.price;
            return <Stack spacing={0.5} sx={{ py: 1 }} direction={"row"} alignItems="center">{price ? <Typography color="text.secondary">${price.toFixed(2)}</Typography> : <Typography color="text.secondary">TBD</Typography>}</Stack>;
        },
        width: 100
    },
    {
