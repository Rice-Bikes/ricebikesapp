...
describe('CreateTransactionDropdown Component', () => {
  ...
  test('renders New Transaction button', () => {
    render(
      <CreateTransactionDropdown user={mockUser} />,
  ...
  test('selects transaction type and opens form', async () => {
    ...
    // Click the New Transaction button to open dropdown
    fireEvent.click(screen.getByText('New Transaction'))
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Transaction Type: Inpatient')).toBeInTheDocument()
      expect(screen.getByText(`User ID: ${mockUser.user_id}`)).toBeInTheDocument()
    })
  })
  ...
  test('creates transaction and navigates when form is submitted', async () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith('/transaction-details/new-transaction-123?type=Inpatient')
  })
