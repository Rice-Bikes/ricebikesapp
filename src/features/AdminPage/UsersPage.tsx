...
const UsersPage: React.FC = () => {
  ...
  const handleRoleChange: (event: SelectChangeEvent<string[]>) => void = (
    ...
    const idArray = Array.isArray(event.target.value)
      ? event.target.value
      : [event.target.value];
  ...
  // Mutation-based checker for the currently-selected user's roles.
  // We use a mutation here (like ManageRoles) so callers can imperatively re-run the fetch
  // after attach/detach operations.
  const checkUserRoles = useMutation({
    mutationKey: ["checkUserRoles"],
    mutationFn: (user_id: string) => DBModel.fetchRolesForUser(user_id),
    onSuccess: (data: Role[]) => {
      // update both the role objects and the selectedRoleIds atomically to avoid a blink/race
      const rolesArr = data || [];
      setUserRoles(rolesArr);
      setSelectedRoleIds(rolesArr.map((r) => r.role_id));
    },
    onError: (error) => {
  ...
  const attachUserToRole = useMutation({
    mutationFn: ({ role_id, userId }: AttachRoleParams) =>
      DBModel.attachRole(userId, role_id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["users"],
      });
      // Re-fetch the user's roles after an attach.
      // trigger the mutation-based fetch (will populate userRoles via onSuccess)
  ...
  const detachUserFromRole = useMutation({
    mutationFn: ({ role_id, userId }: AttachRoleParams) =>
      DBModel.detachRole(userId, role_id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["users"],
      });
      // Re-fetch the user's roles after a detach.
      // trigger the mutation-based fetch (will populate userRoles via onSuccess)
      checkUserRoles.mutate(userId || "");
  ...
  const handleEdit = (user: User) => {
    // Set the selected user ID first so the useQuery for userRoles becomes enabled.
    ...
    // Trigger an immediate fetch for user roles after opening the dialog.
    // Use a short timeout to ensure state updates have propagated.
    setTimeout(() => {
      // use the mutation-based fetch so onSuccess will populate `userRoles` and `selectedRoleIds`
        ...
        <Box sx={{ padding: "2rem", width: "400px" }}>
          ...
          <Stack spacing={2}>
            ...
            <FormControl fullWidth>
              ...
              <Select
                sx={{ display: userId === null ? "none" : "inherit" }}
                labelId="roles-label"
                multiple
                ...
                renderValue={(selected: string[]) => {
                  const selectedRoles = selected
                    .map(
                      (role_id) =>
                        roles.find((r) => r.role_id === role_id)?.name,
                    )
                    .filter(Boolean);
