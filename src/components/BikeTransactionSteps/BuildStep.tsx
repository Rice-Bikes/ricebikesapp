...
const BUILD_TASKS = [
  {
    id: "front_wheel",
    label:
      "Install front wheel with correct tire tread direction, tighten with 15mm wrench",
    repair_id: "00000000-671a-bba2-3810-420b4cca4cd3",
  },
  {
    id: "pedals",
    label:
      "Install pedals (L/R marked, left is reverse threaded), apply grease, tighten to ~10nm",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec05a4",
  },
  {
    id: "chain_tension",
    label:
      'Tension chain using dropout bolts (0.25" up/down motion, slightly loose better)',
    repair_id: "00000000-6349-c2c0-da5c-a7275bb9932f",
  },
  {
    id: "stem",
    label:
      "Apply fiber grip, install stem to max height mark, ensure cable routing, tighten to 22nm",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec0597",
  },
  {
    id: "headset",
    label:
      "Adjust headset (loosen both nuts, hand-tighten bottom until no play, lock top nut)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec058d",
  },
  {
    id: "hubs",
    label: "Check hubs for play (should have none)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec05b5",
  },
  {
    id: "handlebars",
    label: "Rotate bars to rise up and sweep back, tighten to 14nm",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec0592",
  },
  {
    id: "brake_levers",
    label:
      "Position brake levers against grips, tighten moderately (should rotate in crash)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec0589",
  },
  {
    id: "true_wheels",
    label: "True wheels (no sudden wobbles, can be done with wheels on bike)",
    repair_id: "00000000-6025-9c41-654c-0a0dbd6d3847",
  },
  {
    id: "brake_calipers",
    label:
      "Center calipers, apply triflow to pivots, use threadlocker on pads (4nm)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec058b",
  },
  {
    id: "brake_adjustment",
    label:
      "Adjust brake cable tension (lock wheel with less than half lever travel)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec0588",
  },
  {
    id: "seatpost_saddle",
    label:
      "Grease and install seatpost, clamp down on seat tube, ensure bolts snug, level saddle with ground (6-8nm)",
    repair_id: "00000000-5bbd-448f-9093-87ce5113d172",
  },
  {
    id: "tire_pressure",
    label:
      "Inflate tires to slightly under max pressure (if max 90, use 70-80)",
    repair_id: "00000000-671a-bad6-3810-420b4cca4c8f",
  },
  {
    id: "reflectors",
    label:
      "Tighten reflectors straight and out of way for easy seat adjustment",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec05b9",
  },
  {
    id: "shifting",
    label: "Dial in shifting with index adjustments (geared bikes only)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec0598",
  },
  {
    id: "fenders_rack",
    label:
      "Ensure fenders solid (no rubbing), rack secured, chain guard solid (geared bikes)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec05a3",
  },
  {
    id: "bottom_bracket",
    label:
      "Check bottom bracket tightness (non-drive side nut is reverse threaded)",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec0594",
  },
  {
    id: "test_ride",
    label: "TEST RIDE THE BIKE - must perform like brand new",
    repair_id: "00000000-5bbd-407f-9093-87ce5113d16e",
  },
  {
    id: "final_cleanup",
    label:
      "Add owner's manual to desk stack, discard silver tool, no price tag",
    repair_id: "00000000-5a8c-cd91-4459-8272c1ec059e",
  },
];
...
export const BuildStep: React.FC<BuildStepProps> = ({ onStepComplete }) => {
  ...
  const { data: transactionDetails } = useQuery({
    ...
    queryFn: () =>
      transaction_id
        ? DBModel.fetchTransactionDetails(transaction_id, "repair")
        : Promise.resolve([]),
  ...
  const { data: transaction } = useQuery({
    ...
    queryFn: () =>
      transaction_id
        ? DBModel.fetchTransaction(transaction_id)
    ...
    const initializeBuildTasks = async () => {
      ...
      if (transactionDetails && transactionDetails.length > 0) {
        ...
        const duplicateCount =
          transactionDetails.length - uniqueBuildTaskDetails.length;
        console.log(`Found ${duplicateCount} duplicate transaction details`);

        // Delete duplicates from the database
        if (duplicateDetails.length > 0) {
          console.log(
            `Deleting ${duplicateDetails.length} duplicate transaction details...`,
          );
          try {
            await Promise.all(
              duplicateDetails.map(async (duplicate) => {
                if (duplicate.transaction_detail_id) {
                  // Delete the duplicate transaction detail from the database
                  console.log(
                    `Deleting duplicate transaction detail: ${duplicate.transaction_detail_id} (repair_id: ${duplicate.repair_id})`,
                  );
                  await DBModel.deleteTransactionDetails(
                    duplicate.transaction_detail_id,
                  );
                }
              }),
            );
            ...
            console.log(
              `Successfully deleted ${duplicateDetails.length} duplicate transaction details`,
            );
            ...
            console.error(
              "Error deleting duplicate transaction details:",
              error,
            );
          }
        }
        ...
        setBuildTasks((prevTasks) =>
          prevTasks.map((task) => {
            const existingDetail = uniqueBuildTaskDetails.find(
              (detail) => detail.repair_id === task.repair_id,
            );
            return {
              ...task,
              completed: existingDetail
                ? existingDetail.completed || false
                : false,
            };
          }),
        );
        ...
        const missingTasks = BUILD_TASKS.filter(
          (task) => !repairToDetailMap.has(task.repair_id),
        );

        console.log(
          "Missing tasks that need to be created:",
          missingTasks.map((t) => ({ id: t.id, repair_id: t.repair_id })),
        );
  ...
  const handleTaskToggle = async (taskId: string) => {
    ...
    try {
      ...
      if (existingDetail && existingDetail.transaction_detail_id) {
        ...
        console.error(
          "No existing transaction detail found for task:",
          task.id,
          "repair_id:",
          task.repair_id,
        );
        ...
        const duplicates =
          transactionDetails?.filter((d) => d.repair_id === task.repair_id) ||
          [];
        if (duplicates.length > 0) {
          console.warn(
            `Found ${duplicates.length} duplicate entries for repair_id ${task.repair_id}, using first one`,
          );
          await DBModel.updateTransactionDetails(
        ...
        notifyBuildReady(
          ...
          .then((success) => {
            if (success) {
              ...
              console.warn(
                "Build ready notification failed, but build process continues",
              );
            }
          })
          .catch((error) => {
  ...
  const handleStartBuild = async () => {
    ...
    try {
      ...
      if (transaction) {
        ...
        console.log(
          "Build process started - transaction marked as actively being built",
        );
      }
    } catch (error) {
  ...
  const handleAdvanceStep = async () => {
    if (canAdvance) {
      ...
      if (transaction_id) {
        console.log(
          "Build and inspection phase completed! Sending inspection notification...",
        );
        ...
        notifyInspectionComplete(
          ...
          .then((success) => {
            if (success) {
              ...
              console.warn(
                "Inspection complete notification failed, but workflow continues",
              );
            }
          })
          .catch((error) => {
    ...
    <Box>
      ...
      {!hasBuildStarted && (
        <Card sx={{ mb: 3, bgcolor: "warning.50" }}>
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              The bike has arrived in the shop and is ready to be built. Click
              the button below to start the build process and move this
              transaction into the active "Main Transactions" queue.
            </Typography>
            <Button
              variant="contained"
              color="warning"
      ...
      <Card sx={{ mb: 3, opacity: hasBuildStarted ? 1 : 0.6 }}>
        <CardContent>
          ...
          >
            <Typography variant="h6">Build & Inspection Tasks</Typography>
            {!hasBuildStarted && (
              <Typography variant="body2" color="text.secondary">
          ...
          <List sx={{ py: 0 }}>
            {buildTasks.map((task, index) => (
              <ListItem
                ...
                onClick={
                  hasBuildStarted ? () => handleTaskToggle(task.id) : undefined
                }
              >
                <ListItemIcon>
                  <Checkbox
      ...
      {canAdvance && (
        <Alert severity="success" sx={{ mb: 3 }}>
          <Typography variant="body2">
            ✅ All build and inspection tasks completed successfully! Ready to
      ...
      >
        <Typography variant="body2" color="text.secondary">
          Step 2 of 5: Build & Inspection ({completedTasks} of {totalTasks}{" "}
          tasks completed)
        </Typography>
