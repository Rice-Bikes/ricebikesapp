...
describe('Notes attribution integration', () => {
    test('inserting a paragraph marks its top-level node as dirty and records user', async () => {
        ...
        render(
            <EditorApp user={mockUser as any} initialValue={''} onSave={onSave} />,
        ...
        await waitFor(() => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            if (!(window as any).__currentLexicalEditor) throw new Error('editor not mounted yet')
        })
        ...
        await waitFor(() => {
            ...
            if (!meta.dirtyKeys || meta.dirtyKeys.length === 0) throw new Error('dirty keys not set yet')
            ...
            if (!meta.dirtyKeys.includes(createdKey)) throw new Error('createdKey not in dirty keys')
        }, { timeout: 5000 })

        const finalMeta = editor.__getAttributionMeta()
        expect(finalMeta.lastEditedBy).toEqual({ id: 'vitest-user', name: 'Vitest Tester' })
