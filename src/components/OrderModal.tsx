...
const OrderModal: React.FC<OrderModalProps> = ({ order }) => {
  ...
  const { data: closestOrder } = useQuery(DBModel.getClosestFutureOrderQuery());
  const { data: user } = useUser();
  // Use the provided order or the closest future order
  const orderToUse = order || closestOrder;

  const defaultOrderedBy = user ? `${user.firstname} ${user.lastname}` : "";
  const [form, setForm] = useState({
    supplier: orderToUse?.supplier || "",
    ordered_by: orderToUse?.ordered_by || defaultOrderedBy,
  ...
  const canModify =
    !!user &&
    (user.permissions?.some((perm) => perm.name === "canSetOrderDate") ||
      user.username === "cjg8");
  ...
  const createOrderMutation = useMutation({
    mutationFn: (newOrder: Omit<Order, "order_id" | "estimated_delivery">) =>
      DBModel.createOrder({
        ...newOrder,
        estimated_delivery: newOrder.order_date
          ? addDays(newOrder.order_date, 5).toISOString()
  ...
  const updateOrderMutation = useMutation({
    mutationFn: (updatedOrder: Order) =>
      DBModel.updateOrder({
        ...updatedOrder,
        estimated_delivery: updatedOrder.order_date
          ? addDays(new Date(updatedOrder.order_date), 5).toISOString()
          : addDays(new Date(), 5).toISOString(),
      }),
    onSuccess: () => {
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Stack direction={"row"} sx={{ mb: 0 }} alignItems={"center"}>
        ...
        >
          Next Order Date:{" "}
          {orderToUse?.order_date
            ? new Date(orderToUse.order_date).toLocaleDateString()
            : "N/A"}{" "}
        </Button>
        <Button
          variant="contained"
        ...
        >
          Next Delivery:{" "}
          {orderToUse?.estimated_delivery
            ? new Date(orderToUse.estimated_delivery).toLocaleDateString()
            : "N/A"}{" "}
        </Button>
      </Stack>
      <Dialog
        ...
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setForm((prev) => ({ ...prev, ordered_by: e.target.value }))
              }
              required
              disabled={!canModify}
              fullWidth
            ...
            <DatePicker
              ...
              onChange={(newValue: Date | null) =>
                setForm((prev) => ({ ...prev, order_date: newValue }))
              }
              disabled={!canModify}
              slotProps={{
                textField: {
